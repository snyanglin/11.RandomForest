...
if __name__ == "__main__":
    ...
    for i, pair in enumerate(feature_pairs):
        ...
        model.fit(x_train, y_train)#fit元素始终是一个元素对应一个标签的格式。如果不是需将x转换成每行是一个元素的格式

        N, M = 500, 500  # 横纵各采样多少个值
        x1_min, x2_min = x_train.min() #找出x_train中两列中各自的最小值
        x1_max, x2_max = x_train.max()#找出x_train中两列中各自的最大值
        ...
        plt.scatter(x_train[pair[0]], x_train[pair[1]], c=y_train, s=20,marker='o', edgecolors='k', cmap=cm_dark, label=u'训练集')#画出训练集，圆圈格式
        #plt.scatter(x_test[pair[0]], x_test[pair[1]], c=y_test, s=80, marker='*', edgecolors='k', cmap=cm_dark, label=u'测试集')#画出测试集，*号格式
