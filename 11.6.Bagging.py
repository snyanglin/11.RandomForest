...
if __name__ == "__main__":
    ...
    ridged = Pipeline([('poly', PolynomialFeatures(degree=degree)), ('Ridge', ridge)]) #指定几阶的岭回归；#控制特征的多项式的度
    #ridged = Pipeline([('poly', PolynomialFeatures(degree=6)), ('Ridge', ridge)])  # 指定几阶的岭回归；#控制特征的多项式的度
    bagging_ridged = BaggingRegressor(ridged, n_estimators=n_estimators, max_samples=max_samples)#岭回归取50个决策树，相当于用岭回归当成决策树做bagging算法
    dtr = DecisionTreeRegressor(max_depth=9)
    regs = [
        ('DecisionTree', dtr),
        ('Ridge(%d Degree)' % degree, ridged),
        ('Bagging Ridge(%d Degree)' % degree, bagging_ridged),
        ('Bagging DecisionTree', BaggingRegressor(dtr, n_estimators=n_estimators, max_samples=max_samples))]#将dtr做50棵树的随机森林
