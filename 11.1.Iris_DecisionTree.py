...
if __name__ == "__main__":
    ...
    x = data[list(range(4))] #选择列数
    # y = pd.Categorical(data[4]).codes
    y = LabelEncoder().fit_transform(data[4]) #将不同的类别（英文名）转化成1,2,3
    # 为了可视化，仅使用前两列特征
    x = x.iloc[:100, :2]
    y = y[:100]
    # x = x[[0,1]]
    ...
    model = DecisionTreeClassifier(criterion='gini') #‘gini’ or ‘entropy’ (default=”gini”)，前者是基尼系数，后者是信息熵。
    ...
    # 1、输出
    with open('iris.dot', 'w') as f: #try ... finally是一样的
    ...
    cm_light = mpl.colors.ListedColormap(['#A0FFA0', '#FFA0A0', '#A0A0FF'])#设置预测值背景
